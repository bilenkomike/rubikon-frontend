...
// const ProductsPage = () => {
//   const { lang, subcategory, search } = useParams();
...
//   /* ---------------- FETCH SUBCATEGORY (only if slug) ---------------- */
...
//   // const fetchFilters = async () => {
//   //   try {
//   //     const params = search ? `search=${search}` : `subcategory=${subcategory}`;
...
const ProductsPage = () => {
  const { lang, subcategory } = useParams();

  const [subcategoryData, setSubcategoryData] = useState(null);
  const [filtersConfig, setFiltersConfig] = useState(null);
  const [filters, setFilters] = useState({ price: [null, null] });
  const [products, setProducts] = useState([]);
  ...
  useEffect(() => {
    if (subcategory && (!filtersConfig || !filters?.price)) return;
    fetchProducts();
  }, [filters, page, subcategory, filtersConfig]);
  ...
  const buildQueryParams = (filters, page) => {
    ...
    // ✅ Only add price if both ends are valid numbers
    if (
      Array.isArray(filters?.price) &&
      isValidNumber(filters.price[0]) &&
      isValidNumber(filters.price[1])
    ) {
      params.append("price_min", filters.price[0]);
      params.append("price_max", filters.price[1]);
    }
  ...
  return (
    <Container maxWidth={false} sx={{ maxWidth: "1600px", mt: 3, mb: 8 }}>
      ...
      <Box sx={{ display: "flex", gap: 16 }}>
        ...
        <Box flex={1}>
          {loading && (
            <Typography sx={{ mb: 3 }}>Loading products...</Typography>
          )}
